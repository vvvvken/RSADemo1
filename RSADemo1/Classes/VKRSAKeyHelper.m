...
+ (SecKeyRef)privateKeyOfPem:(NSString *)privateKey error:(NSError **)error{
    return [self addPrivateKey:privateKey error:error];
}

+ (SecKeyRef)publicKeyOfPem:(NSString *)publicKey error:(NSError **)error{
    return [self addPublicKey:publicKey error:error];
}
...
+ (SecKeyRef)addPublicKey:(NSString *)key error:(NSError **)error{
    ...
    NSString *tag = @"com.VKRSAKeyHelper.publicKey";
...
+ (SecKeyRef)addPrivateKey:(NSString *)key error:(NSError **)error{
    ...
    //a tag to read/write keychain storage
    NSString *tag = @"com.VKRSAKeyHelper.privateKey";
    NSData *d_tag = [NSData dataWithBytes:[tag UTF8String] length:[tag length]];
